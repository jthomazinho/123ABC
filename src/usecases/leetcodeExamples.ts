...
export class LeetCodeExamples {
  ...
  private static isValidBSTHelper(node: TreeNode | null, min: number, max: number): boolean {
    if (!node) return true;
  ...
  private static dfs(grid: string[][], row: number, col: number): void {
    const rows = grid.length;
    const cols = grid[0].length;
  ...
  private static backtrack(nums: number[], start: number, path: number[], result: number[][]): void {
    result.push([...path]);
   ...
   */
  static createLRUCache(capacity: number) {
